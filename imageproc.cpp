...
void IMG2DLib::colorToGray(IMG2DLib::Mat& colorImage, int contrastValue, int brightValue, int setWhiteVal)
{
	...
	int imgH = colorImage.imageH;
	int channels = colorImage.channels;
	double contrastValueRatio = contrastValue * 0.01;
...
MatPotrace::PointF lerp(const MatPotrace::PointF& a, const MatPotrace::PointF& b, double t)
{
	return a + (b - a) * t;
}

// evaluate a point on a bezier-curve. t goes from 0 to 1.0
MatPotrace::PointF bezier(const MatPotrace::PointF& a, const MatPotrace::PointF& b, const MatPotrace::PointF& c, const MatPotrace::PointF& d, double t)
{
	MatPotrace::PointF ab, bc, cd, abbc, bccd;
...
potrace_bitmap_t* bitmapFromImage(const IMG2DLib::Mat& image,
	int threshold)
{
...
MatPotrace::PolygonF polygonFromPath(potrace_path_t* path,
	int bezierPrecision)
{
	MatPotrace::PolygonF poly;
	if (!path) return poly;
	...
	for (int i = 0; i < n; ++i)
	{
		...
		{
		case POTRACE_CORNER:
			poly.push_back(MatPotrace::PointF(c[i][1].x, c[i][1].y));
			poly.push_back(MatPotrace::PointF(c[i][2].x, c[i][2].y));
			break;
		case POTRACE_CURVETO:
		{
			MatPotrace::PointF pa, pb, pc, pd;
			pa = poly.empty() ? MatPotrace::PointF(c[n - 1][2].x, c[n - 1][2].y) :
				poly.back();
			pb = MatPotrace::PointF(c[i][0].x, c[i][0].y);
			pc = MatPotrace::PointF(c[i][1].x, c[i][1].y);
			pd = MatPotrace::PointF(c[i][2].x, c[i][2].y);
			for (int i = 1; i <= bezierPrecision; ++i)
			{
				poly.push_back(bezier(pa, pb, pc, pd, static_cast<double>(i) / bezierPrecision));
			}
		}
		break;
		}
	...
	if (!poly.empty() && poly.front().samePoint(poly.back()))
	{
		poly.pop_back();
	}
...
std::vector<MatPotrace::PolygonF> holesFromPath(potrace_path_t* path,
	int bezierPrecision)
{
	std::vector<MatPotrace::PolygonF> holes;
	...
	while (path)
	{
		if (path->sign == '-')
		{
			holes .push_back(polygonFromPath(path, bezierPrecision));
		}
		path = path->sibling;
	}
	return holes;
...
MatPotrace::Polygon tracedPolygonFromPath(potrace_path_t* path,
	int bezierPrecision)
{
	MatPotrace::Polygon def;
...
std::vector<MatPotrace::Polygon> tracedPolygonsFromPath(potrace_path_t* path,
	int bezierPrecision)
{
	std::vector<MatPotrace::Polygon> defs;
	while (path)
	{
		if (path->sign == '+')
		{
			defs .push_back(tracedPolygonFromPath(path,
				bezierPrecision));
		}
		path = path->next;
	}
...
bool MatPotrace::trace(const IMG2DLib::Mat& image)
{
...
std::vector<MatPotrace::Polygon> MatPotrace::polygons() const
{
	return m_meshDefs;
}

void MatPotrace::setAlphaMax(double alphaMax)
{
	m_alphaMax = alphaMax;
}

void MatPotrace::setTurdSize(int turdSize)
{
	m_turdSize = turdSize;
}

void MatPotrace::setCurveTolerance(double tolerance)
{
	m_curveTolerance = tolerance;
}

void MatPotrace::setThreshold(int threshold)
{
	m_threshold = threshold;
}

void MatPotrace::setBezierPrecision(int precision)
{
	if(precision > 0)
